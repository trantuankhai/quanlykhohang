...
function validateAddProduce(name,sls,origin_price,sell_price){
    if (name.length == 0) {
        $('.ajax-error-ct').html('Vui lòng nhập tên sản phẩm.').parent().fadeIn().delay(1000).fadeOut('slow');
        return false;  
     }else if(sls.length == 0){
        $('.ajax-error-ct').html('Vui lòng nhập số lượng sản phẩm.').parent().fadeIn().delay(1000).fadeOut('slow');
        return false;  

     }else if(origin_price.length == 0 || origin_price == 0){
        $('.ajax-error-ct').html('Vui lòng nhập giá gốc sản phẩm.').parent().fadeIn().delay(1000).fadeOut('slow');
        return false;  

     }else if(sell_price.length == 0 || sell_price == 0 ){
        $('.ajax-error-ct').html('Vui lòng nhập giá bán sản phẩm.').parent().fadeIn().delay(1000).fadeOut('slow');
        return false;  
...
function cms_add_product(type) {
    ...
    if(validateAddProduce($name,$sls,$origin_price,$sell_price) == true){
               var $data = {
            'data': {
                'prd_name': $name,
                'prd_code': $code,
...
function cms_showalll_inventory($page){
    $store_id = $('#store-id').val();
...
function cms_set_current_week() {
    ...
    lastday = new Date(curr.setDate(curr.getDate()+6)).toISOString().split('T')[0];
    $('#search-date-from').val(firstday);
    $('#search-date-to').val(lastday);
}
function cms_set_last_date(){
    var curr = new Date; 
